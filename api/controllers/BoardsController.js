 ...
 */
 ...
 module.exports = {
 	...
 	joinGame: function(req,res) {
 		...
 		Board.find({ where: { id: req.param('room'), isAvailable :true},
 			limit: 1}).exec(function(err,board) {
 					console.log(board);
 				Board.update( { id: req.param('room')},
 				{
 					joinee: socket.id,
 					isAvailable: false
 					...
 					console.log('printing borad');
 						...
 						if(updatedBoards.length  > 0){
 								...
 								sails.io.to(updatedBoards[0].id).emit('joinGame',{message: 'Joined', board: updatedBoards});
 						}else{
 		...
 		createGame: function(req,res) {
 			...
 			Board.create({
 				...
 				console.log('Joining to created :'+board.id);
